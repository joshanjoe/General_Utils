...
REM #--------------------------------------------------------------------------------#
REM # Workspace Environment Setup [internally uses wasenvsetup.py] - wrote in MS DOS #
REM # wasenvsetup.bat                                                                #
...
copy /Y .\wasenvsetup.py %sys_drive%\environment-setup-temp-1234
...
cmd /c "%WAS_HOME%\bin\wsadmin.bat" -lang jython -conntype SOAP -host 127.0.0.1 -port %SOAP_PORT% -f %sys_drive%\environment-setup-temp-1234\wasenvsetup.py
